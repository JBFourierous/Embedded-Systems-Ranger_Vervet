...
uint32_t led_matrix_message[(N_LEDS+1)*LED_BYTE];

/* Inner functions to handle the single ws2812 LED of the matrix, in part wrapped by turn_off_led_matrix and turn_on_led_matrix */
...
static void ws2812_applyColors() {
	HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_1, (uint32_t*)led_matrix_message,
